...
unsigned int height(node *p) {              // Возврат высоты
    return p ? p->height : 0;               // Если p не нулевой указатель - возвращаем его высоту, иначе возвращаем 0
}
...
void fixheight(node *p) {                   // Восстановление корректного значения поля height заданного узла
    unsigned int hl = height(p->left);  // Высота левого поддерева
    unsigned int hr = height(p->right); // Высота правого поддерева
...
node *balance(node *p) {                    // Балансировка узла p
    fixheight(p);
    if (bfactor(p) == 2) {                  // Разность между высотами левого и правого поддерева равна 2
...
int main() {
    ...
    menu(); // вывод пользовательского меню
    int operation; // переменная для ввода номера операции
    cin >> operation;
    while (true) {
